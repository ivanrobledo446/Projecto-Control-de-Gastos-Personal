  ...
  const loadTransactions = useCallback(async (m: number, y: number) => {
    try {
      const res = await fetch(
        `/api/transactions?month=${m}&year=${y}&kind=EXPENSE`
      );
      if (!res.ok) throw new Error(await readApiError(res));
      const data = await res.json();
      setTransactions(Array.isArray(data) ? data : []);
    } catch (e: any) {
      toast.error(e?.message ?? 'No se pudieron cargar los gastos.');
      setTransactions([]);
    }
  }, []);
  ...
  const saveEditedTransaction = useCallback(async () => {
    ...
    try {
      ...
      if (editedMonth !== month || editedYear !== year) {
        toast.info(`El gasto se movió a ${editedMonth}/${editedYear}`);
      }

      closeEditModal();
      await loadTransactions(month, year);
    } catch (e: any) {
      toast.error(e?.message ?? 'No se pudo actualizar el gasto.');
    } finally {
      setEditSaving(false);
    }
  }, [
    ...
    <main className="space-y-6">
      <header className="space-y-1">
        <h1 className="text-2xl font-bold">
          Gastos {monthLabel} del {year}
        </h1>
        <p className="text-sm text-[var(--muted)]">
          Registrá, editá y eliminá gastos. Deslizá para acciones rápidas.
        </p>
      </header>
      ...
      {editingTxId ? (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4">
          <div className="w-full max-w-md rounded-xl border border-[var(--border)] bg-[var(--bg)] text-[var(--text)] p-5">
            <h3 className="font-semibold text-lg mb-3">Editar gasto</h3>
